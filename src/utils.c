...
{
  double durationMs = ((double)(end - start) / CLOCKS_PER_SEC) * 1000.0; // menghitung selisih waktu

  // cetak dalam satuan milidetik
  if (durationMs < 1000.0)
    printf("Waktu pengurutan: %.2f ms\n", durationMs);

  // jika 1 detik atau lebih, konversi ke detik terlebih dahulu
  else
  {
...
void extractCollectionName(const char *path, char *outName, size_t maxLen)
{
  ...
  // validasi  jika '/' tidak ketemu, cari karakter backslash '\' (format Windows)
  if (!fileName)
    fileName = strrchr(path, '\\');

  // validasi jika pemisah direktori ditemukan, lewati karakter tersebut agar menunjuk langsung ke nama file
  if (fileName)
    fileName++;

  // validasi
  else
    fileName = path; // jika tidak ada pemisah sama sekali, berarti input adalah nama filenya

  // sekarang `fileName` sudah berisi string nama file, contoh: "docword.kos.txt"
...
void normalizePaths(const char *inputPath, char *docwordPath, char *vocabPath, size_t maxLen)
{
  ...
  if (dirLen > 0)
  {
    snprintf(docwordPath, maxLen, "%.*sdocword.%s.txt", (int)dirLen, inputPath, collection);
    snprintf(vocabPath, maxLen, "%.*svocab.%s.txt", (int)dirLen, inputPath, collection);
  }
  else
  {
